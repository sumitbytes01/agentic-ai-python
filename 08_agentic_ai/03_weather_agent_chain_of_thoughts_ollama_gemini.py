...
SYSTEM_PROMPT = """
    ...
    - Only run one step at a time. this is a strict rule.
    - The sequence of steps is START (where user gives an input), PLAN (That can be multiple times) and finally OUTPUT (which is going to the displayed to the user).
    - also the result expected from the ALM is a single response at a time. multiple steps response is not allowed.
...
while True:
    ...
    # Create prompt text by joining the roles and content for Gemini API
    ...
    while True:
        ...
        # parse JSON from raw_result for step and other fields using your preferred JSON parsing logic
        # example (pseudo-code):
        # parsed_result = MyOutputFormat.parse_raw(raw_result)
        # Using pydantic BaseModel for validation and parsing:
        try:
            parsed_result = MyOutputFormat.parse_raw(raw_result)
